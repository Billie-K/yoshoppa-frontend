...
const Neon: React.FC<NeonProps> = ({ product, className, shop }) => {
  ...
  return (
    <article
      className={cn(
        'product-card cart-type-neon rounded h-full bg-light overflow-hidden transition-all duration-200 transform hover:-translate-y-1',
        className
      )}
    >
      <div
      ...
      <header className="p-3 md:p-6">
        ...
        <h4
          className="text-xs md:text-sm italic text-gray-400 mt-1 truncate mb-4 cursor-pointer"
          onClick={handleProductQuickView}
        >
          {shop}
        </h4>
        {/* End of shop title title */}
      </header>
    </article>
  );
};
