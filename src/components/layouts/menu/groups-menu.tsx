...
const GroupsMenu: React.FC<GroupsMenuProps> = ({
  ...
  return (
    ...
    >
      <Menu.Button
        className={cn(
          'flex items-center shrink-0 text-sm md:text-light font-semibold h-11 focus:outline-none text-heading xl:px-4',
          {
            'bg-green-500 rounded-lg px-3':
              variant === 'minimal',
      ...
      >
        {({ open }) => (
          <>
            {variant === 'colored' || variant === 'minimal' && selectedMenu?.icon && (
            ...
            {groups?.map(({ id, name, slug, icon }) => (
              <Menu.Item key={id}>
                {({ active }) => (
                  ...
                  >
                    {icon && variant === 'colored' || variant === 'minimal' && (
